...
def init(self, panel_id, button_id, button_label, questions):
super().init(title=f"{button_label[:35]} Ticket")
...
def init(self, panel_id, custom_id, label, questions):
super().init(style=discord.ButtonStyle.primary, label=label[:80], custom_id=f"ticket:{panel_id}:{custom_id}")
self.panel_id = panel_id
...
def init(self, panel_id, buttons_data):
...
def init(self, guild_id, panel_id=None):
super().init(timeout=900)
...
super().init(placeholder="Select a ticket panel...", options=options or [discord.SelectOption(label="No panels", value="none")])
...
def init(self, guild_id, panel_id):
super().init(label="Back", style=discord.ButtonStyle.secondary, emoji="↩️")
self.guild_id = guild_id
self.panel_id = panel_id
async def callback(self, interaction):
...
def init(self, guild_id, panel_id, action):
...
super().init(placeholder="Select a button...", options=options or [discord.SelectOption(label="No buttons", value="none")])
async def callback(self, interaction):
cid = self.values[0]
if cid == "none":
...
def init(self, guild_id, panel_id):
super().init(title="Add Ticket Button")
...
def init(self, guild_id, panel_id, cid):
data = get_panel(guild_id, panel_id)["buttons"][cid]
super().init(title="Edit Ticket Button")
...
def init(self, guild_id, panel_id, panel):
super().init(title="Edit Ticket Panel")
self.guild_id, self.panel_id = guild_id, panel_id
...
def init(self, guild_id, panel_id):
panel = get_panel(guild_id, panel_id)
super().init(title="Edit Ticket Message")
self.guild_id, self.panel_id = guild_id, panel_id
...
def init(self, category, role):
super().init(title="Create Ticket Panel")
